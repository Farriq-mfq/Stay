...
const onChangeProfilePictureService = async (file) => {
    ...
    const response = await axios.post('/pegawai/modules/profile/update-profile-picture', formData, {
        headers: {
                ...
                <div class="flex flex-column gap-2">
                    ...
                    <div class="p-card shadow-1 flex flex-column justify-content-center p-3 border-round-xl">
                        <div class="flex flex-column gap-3">
                            ...
                            <div class="flex flex-column gap-1">
                                <span class="text-sm text-500">Username / NIP</span>
                                <span class="text-lg text-color">{{ account.username }}</span>
                            </div>
                            <div class="flex flex-column gap-1">
                                <span class="text-sm text-500">Jabatan</span>
                                <span class="text-lg text-color">{{ account.position }}</span>
                            </div>
                            <div class="flex flex-column gap-1">
                                <span class="text-sm text-500">Kelompok</span>
                                <span class="text-lg text-color">{{ account.group }}</span>
                            </div>
                        </div>
                    </div>
                </div>
