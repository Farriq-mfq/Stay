...
watch(
    () => sessionData.value.session_role_type,
    () => {
        selectedGroup.value = [];
    }
);
...
watch(
    () => dataUpdateSession.value.session_role_type,
    () => {
        const mappingSelectedGroup = selectedGroup.value.map((item) => item.value);
        if (dataUpdateSession.value.session_role_type == 'SISWA') {
        ...
        } else if (dataUpdateSession.value.session_role_type == 'PEGAWAI') {
            if (dataGroups.value) {
...
// meeting session
const dialogMeetingSession = ref(false);
const dataMeetingSession = ref(null);
const showDialogMeetingSession = (data) => {
    dialogMeetingSession.value = true;
    dataMeetingSession.value = data;
};

const onCloseDialogMeetingSession = () => {
...
<template>
    <div class="grid">
        <div class="col-12">
                ...
                >
                    ...
                    <template #empty>
                        <div class="flex justify-content-center p-4 gap-3 align-items-center">
                            ...
                            <span> Data session masih kosong </span>
                        </div>
                    </template>
                    <Column expander />
                    ...
                    <Column field="session_role_type" header="Tipe Role">
                        <template #body="{ data }">
                            <Tag v-if="data.session_role_type" severity="info">{{ data.session_role_type }}</Tag>
                            <p v-else>-</p>
                        </template>
                    </Column>
                    <Column field="allow_twice" header="Presensi 2x">
                        <template #body="{ data }">
                            <Chip class="py-0 pl-0 pr-4">
                                <span :class="{ 'bg-primary': data.allow_twice, 'bg-red-400': !data.allow_twice }" class="border-circle w-2rem h-2rem flex align-items-center justify-content-center text-white">
                                    <i :class="{ 'pi pi-check': data.allow_twice, 'pi pi-times': !data.allow_twice }"></i
                                ></span>
                    ...
                    <Column field="count" alignFrozen="right" :frozen="true" header="Gateway">
                        <template #body="{ data }">
                            <Badge>
                                {{ data.gateways.length }}
                            </Badge>
                    ...
                    <Column field="meeting_session" header="Sesi Rapat" width="10rem">
                        <template #body="{ data }">
                            <div class="flex flex-column" v-if="data.meeting_session && data.session_role_type === 'PEGAWAI'">
                                <div class="flex justify-content-between align-items-center gap-4">
                                    <h6 class="m-0 text-sm">Nama Rapat</h6>
                                    <span>{{ data.meeting_session.name }}</span>
                                </div>
                                <Divider />
                                <div class="flex justify-content-between align-items-center gap-4">
                            ...
                            <template #list="slotProps">
                                <div class="card" v-for="(item, index) in slotProps.items" :key="index">
                                    <div class="flex align-items-center gap-3">
                                        <div :class="`${item.status ? 'bg-primary' : 'bg-red-500'} h-1rem w-1rem border-circle`"></div>
            ...
            <!-- create -->
            <Dialog v-model:visible="addSessionDialog" :style="{ width: '450px' }" header="Buat sesi presensi baru" :modal="true" class="p-fluid">
                <div class="field">
                    <label for="role_type"> Sesi Presensi diijinkan untuk ? </label>

                    <div class="flex flex-wrap gap-3 border-1 py-4 px-2 border-round border-primary justify-content-center">
                        <div class="flex align-items-center">
                            <RadioButton v-model="sessionData.session_role_type" inputId="siswa" name="siswa" value="SISWA" />
                    ...
                    <p class="text-red-500" v-if="errorsAddSession && errorsAddSession.session_role_type">
                        {{ errorsAddSession.session_role_type[0] }}
                    </p>
                </div>
                <div class="field">
                ...
                <div class="field">
                    <label for="start_time"> Waktu Mulai (Optional) </label>
                    <Calendar timeOnly id="calendar-24h-mulai" v-model="sessionData.start_time" hourFormat="24" showSeconds :disabled="addSessionLoading" showButtonBar showIcon />
                    <p class="text-red-500" v-if="errorsAddSession && errorsAddSession.start_time">
                        {{ errorsAddSession.start_time[0] }}
                    </p>
                </div>
                <div class="field">
                    <label for="end_time"> Waktu Selesai (Optional) </label>
                    <Calendar timeOnly id="calendar-24h-selesai" v-model="sessionData.end_time" showTime hourFormat="24" :disabled="addSessionLoading" showSeconds showButtonBar showIcon />
                ...
                <div class="field">
                    ...
                    <label for="group" v-else-if="sessionData.session_role_type === 'PEGAWAI'">Pilih Kelompok Pegawai (Optional)</label>
                    <!-- siswa -->
                    <MultiSelect
                        v-if="sessionData.session_role_type == 'SISWA'"
                    ...
                    <!-- pegawai -->
                    <MultiSelect
                        v-else-if="sessionData.session_role_type == 'PEGAWAI'"
                ...
                <template #footer>
                    <Button label="Batal" :disabled="addSessionLoading" severity="danger" icon="pi pi-times" outlined @click.prevent="addSessionDialog = false" />
                    <Button label="Simpan" :loading="addSessionLoading" :disabled="addSessionLoading" icon="pi pi-link" @click="handleAddSession" />
                </template>
            </Dialog>
            <Dialog v-model:visible="showDialogUpdateSession" :style="{ width: '450px' }" header="Edit Sesi Presensi" :modal="true" class="p-fluid" @after-hide="clearUpdateSession">
                <div class="field">
                    <label for="role_type"> Sesi Presensi diijinkan untuk ? </label>
                    <div class="flex flex-wrap gap-3 border-1 py-4 px-2 border-round border-primary justify-content-center">
                        <div class="flex align-items-center">
                            <RadioButton v-model="dataUpdateSession.session_role_type" inputId="siswa" name="siswa" value="SISWA" />
                            <label for="siswa" class="ml-2">Siswa</label>
                        </div>
                        <div class="flex align-items-center">
                            <RadioButton v-model="dataUpdateSession.session_role_type" inputId="pegawai" name="pegawai" value="PEGAWAI" />
                            <label for="pegawai" class="ml-2">Pegawai</label>
                        </div>
                    </div>
                    <p class="text-red-500" v-if="errorsUpdateSession && errorsUpdateSession.session_role_type">
                        {{ errorsUpdateSession.session_role_type[0] }}
                    </p>
                </div>
                <div class="field">
                    <label for="name">Nama</label>
                ...
                <div class="field">
                    <label for="start_time"> Waktu Mulai (Optional) </label>
                    <Calendar
                        id="calendar-24h-mulai"
                        timeOnly
                        showSeconds
                        showIcon
                        v-model="dataUpdateSession.start_time"
                        showTime
                        hourFormat="24"
                        :invalid="errorsUpdateSession && errorsUpdateSession.start_time"
                ...
                <div class="field">
                    <label for="end_time"> Waktu Selesai (Optional) </label>
                    <Calendar
                        id="calendar-24h-selesai"
                        timeOnly
                        showSeconds
                        showIcon
                        v-model="dataUpdateSession.end_time"
                        showTime
                        hourFormat="24"
                        :invalid="errorsUpdateSession && errorsUpdateSession.end_time"
                        :disabled="updateSessionLoading"
                        showButtonBar
                    />
                ...
                <div class="field">
                    <label for="gateways">Gateway (Optional)</label>
                    <SelectGateway role="presence" :defaultValue="dataUpdateSession.gateways" multiple @input="handleChangeUpdateSessionGateway" />
                ...
                <div class="field">
                    <label for="group" v-if="dataUpdateSession.session_role_type === 'SISWA'">Pilih Rombel Siswa (Optional)</label>
                    <label for="group" v-else-if="dataUpdateSession.session_role_type === 'PEGAWAI'">Pilih Kelompok Pegawai (Optional)</label>
                    <!-- siswa -->
                    <MultiSelect
                        v-if="dataUpdateSession.session_role_type == 'SISWA'"
                        :loading="loadingRombel"
                        filter
                        v-model="selectedGroup"
                        :options="dataRombels ? dataRombels.data.data.map((item) => ({ value: item })) : []"
                    ...
                    <!-- pegawai -->
                    <MultiSelect
                        v-else-if="dataUpdateSession.session_role_type == 'PEGAWAI'"
                        :loading="loadingGroup"
                        filter
                        v-model="selectedGroup"
                        :options="dataGroups ? dataGroups.data.data.map((item) => ({ value: item })) : []"
                ...
                <template #footer>
                    <Button label="Batal" :disabled="updateSessionLoading" severity="danger" icon="pi pi-times" outlined @click.prevent="showDialogUpdateSession = false" />
                    <Button label="Update" :loading="updateSessionLoading" :disabled="updateSessionLoading" icon="pi pi-link" @click="handleSubmitUpdateSesion" />
                </template>
            </Dialog>
            <Dialog v-model:visible="showdialogQrCode" :modal="true" @after-hide="clearDialogQrCode">
                <div class="w-full flex flex-column justify-content-center align-items-center text-center" :id="`qrsession-${dataQr.qrcode}`">
                ...
                <template #footer>
                    <Button label="Batal" severity="danger" icon="pi pi-times" outlined @click.prevent="showdialogQrCode = false" />
