...
const getChartPresencesService = async () => {
    ...
    return await axios.get(`/stats/presences/rombel/${session.value.id}?${queryParams}`)
}
...
const {
    ...
    queryKey: ['getChartByRombel', session, selectedDate],
...
<template>
    <div>
        <div class="field">
            ...
            <Calendar view="month" dateFormat="mm/yy" v-if="session" v-model="selectedDate"
                placeholder="Pilih Bulan" class="w-full mt-3" showButtonBar />
