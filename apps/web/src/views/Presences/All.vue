...
<template>
    <div>
        ...
        <div v-if="sessionId" class="mt-3">
            <div class="flex gap-2">
                <Calendar v-model="filterDate" :disabled="isLoading" placeholder="Filter Tanggal Presensi" showButtonBar selectionMode="range" :manualInput="false" />
        ...
        <DataTable
            ...
            :loading="isLoading"
            :value="isLoading ? [] : presences.data.data.items"
            dataKey="id"
            paginator
            :rows="10"
            :filters="filters"
            ...
            currentPageReportTemplate="Showing {first} to {last} of {totalRecords} presences"
            :first="first"
            @page="onPage($event)"
        >
            <template #header>
            ...
            <template #empty>
                <div class="flex justify-content-center p-4 gap-3 align-items-center">
                    ...
                    <span> Data presences masih kosong </span>
                </div>
            </template>
            <!-- <Column expander /> -->
            <Column header="Nama">
            ...
            <Column header="Masuk">
                <template #body="{ data }">
                    {{ data.enter_time ? format(data.enter_time, 'yyyy-MM-dd HH:mm:ss') : '-' }}
                </template>
            </Column>
            ...
            <Column header="Metode">
                <template #body="{ data }">
                    ...
                    <Tag severity="contrast" v-if="data.method === 'manual'">
                        {{ data.method }}
                    </Tag>
