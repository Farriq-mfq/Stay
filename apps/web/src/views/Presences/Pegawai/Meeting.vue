...
const getPegawai = async () => {
    if (!selectedMeetingSession.value && !selectedMeetingSession.value.presence_sessions) return;
    return await axios.get(`/pegawai/${selectedMeetingSession.value.presence_sessions.id}/all`);
};

const { data: pegawai } = useQuery({
    queryKey: ['pegawai-all', selectedMeetingSession],
    queryFn: getPegawai
});
</script>
...
<template>
    <div>
        ...
        <Button v-if="selectedMeetingSession && selectedMeetingSession.presence_sessions" @click="generatePdf" class="mt-3" icon="pi pi-file-pdf" label="Download Pdf" severity="danger" />
        <div v-if="selectedMeetingSession && selectedMeetingSession.presence_sessions" class="border-1 p-5 my-4 surface-border border-round">
            <div class="flex flex-column align-items-center gap-5 p-3 overflow-auto" ref="pdfContent">
                ...
                <div class="w-full flex flex-column align-items-center">
                    <table class="p-datatable p-component p-datatable-gridlines w-full">
                        <thead class="p-datatable-thead">
                            <tr>
                                <th class="p-column-header surface-200">No</th>
                                <th class="p-column-header surface-200">Nama</th>
                                <th class="p-column-header surface-200">NIP</th>
                                <th class="p-column-header surface-200">TTD</th>
                            </tr>
                        </thead>
                        <tbody class="p-datatable-tbody">
                            <tr class="p-datatable-row h-1rem" v-for="(pg, idx) in pegawai.data.data" :key="pg.id">
                                ...
                                <td class="p-datatable-cell" style="max-width: 7rem;overflow: hidden;">{{ pg.username }}</td>
                                <td class="p-datatable-cell w-18rem">
                                    <div class="w-8rem flex justify-content-center sign_container" :class="{ even: (idx + 1) % 2 === 0 }">
                                        <img src="https://www.shutterstock.com/image-vector/signature-certificate-document-vector-transparent-260nw-2346804443.jpg" class="h-5rem w-5rem" alt="sign" />
                                    </div>
                                </td>
                            </tr>
                        </tbody>
