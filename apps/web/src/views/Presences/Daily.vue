    ...
    <div>
        <div class="field">
            ...
            <Dropdown :loading="loadingRombel" v-model="selectedRombel"
                ...
                placeholder="Pilih Rombel" class="w-full mt-3" showClear />
            ...
            <div class="flex gap-2">
                <Button :disabled="isLoading || loadingExport" :loading="isLoading || loadingExport"
                    ...
                    v-if="sessionId && filterDate" />
                <Button severity="info" :disabled="isLoading" :loading="isLoading" icon="pi pi-print" label="Print"
                    @click.prevent="handlePrint" class="mt-3" v-if="sessionId && filterDate" />
                <Button :disabled="isLoading || loadingExport" :loading="isLoading || loadingExport"
                ...
                <Button icon="pi pi-refresh" outlined label="Reset" @click.prevent="resetAll" class="mt-3"
                    severity="danger" v-if="sessionId && filterDate" />
            </div>

        </div>

        <div ref="componentPrintRef" class="overflow-x-auto"
            v-if="filterDate && presences && presences.data.data.rombel === null">
        ...
        <div ref="componentPrintRef"
            v-if="sessionId && filterDate && presences && presences.data.data.rombel != null && selectedRombel">
            <div class="detail-session">
                <h3>
                    {{ detailSession.name }}
                </h3>
