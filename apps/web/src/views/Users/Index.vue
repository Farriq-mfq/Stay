...
const { mutateAsync: addUser, isPending: addUserLoading } = useMutation({
    mutationKey: ['addUser'],
...
const { mutateAsync: updateUser, isPending: updateUserLoading } = useMutation({
...
const { mutateAsync: updateUserPassword, isPending: updateUserPasswordLoading } = useMutation({
            ...
            <div class="card">
                ...
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            <Button label="Tambah" v-if="$can('users:create')" icon="pi pi-plus" class="mr-2" @click.prevent="showDialogUser = true" />
                        </div>
                    </template>
                </Toolbar>
                <DataTable
                    ...
                    :loading="isLoading"
                    :value="isLoading ? [] : users.data.data.items"
                    dataKey="id"
                    paginator
                    :rows="10"
                    :filters="filters"
                ...
                >
                    ...
                    <template #empty>
                        <div class="flex justify-content-center p-4 gap-3 align-items-center">
                            ...
                            <span> Data users masih kosong </span>
                        </div>
                    </template>
                    <Column field="name" header="Nama"> </Column>
                    <Column field="username" header="Username"> </Column>
                    <Column field="roles" header="Hak Akses">
                        <template #body="{ data }">
                            {{ data.roles.name }}
                        </template>
                    </Column>
                    ...
                    <Column headerStyle="width:4rem">
                        <template #body="{ data }">
                            <div class="flex gap-2 mt-1">
                                <Button icon="pi pi-pencil" v-if="$can('users:update')" @click.prevent="handleShowDialogUpdateUser(data)" />
                                <Button :loading="deleteUserPending" v-if="$can('users:delete')" :disabled="deleteUserPending" severity="danger" @click.prevent="confirmDeleteUser(data.id)" icon="pi pi-trash" />
                                <Button
                                    :loading="updateUserPasswordLoading"
                                    v-if="$can('users:update-password')"
                                    :disabled="updateUserPasswordLoading"
                                    outlined
                                    severity="warning"
                                    @click.prevent="handleShowDialogUpdateUserPassword(data)"
                                    icon="pi pi-key"
                                />
                            </div>
                        </template>
                ...
                <!-- created -->
                <Dialog v-model:visible="showDialogUser" :style="{ width: '450px' }" header="Tambah User Baru" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="name">Nama</label>
                        <InputText id="name" :disabled="addUserLoading" :invalid="errorsAddUser && errorsAddUser.name" required="true" autofocus v-model="userAddData.name" />
                    ...
                    <div class="field">
                        ...
                        <InputText id="username" :disabled="addUserLoading" :invalid="errorsAddUser && errorsAddUser.username" required="true" autofocus v-model="userAddData.username" />
                    ...
                    <div class="field">
                        ...
                        <InputText id="password" :disabled="addUserLoading" :invalid="errorsAddUser && errorsAddUser.password" required="true" autofocus v-model="userAddData.password" />
                    ...
                    <div class="field">
                        ...
                        <InputText id="confirmation_password" :disabled="addUserLoading" :invalid="errorsAddUser && errorsAddUser.confirmation_password" required="true" autofocus v-model="userAddData.confirmation_password" />
                    ...
                    <div class="field">
                        ...
                        <Dropdown
                            v-model="userAddData.roles"
                            :loading="rolesLoading"
                            optionLabel="name"
                            :options="roles.data.data.map((item) => ({ value: item.id, name: item.name }))"
                            placeholder="Pilih Hak Akses"
                            class="w-full"
                            :invalid="errorsAddUser && errorsAddUser.roles"
                            :disabled="addUserLoading"
                        />
                        <p class="text-red-500" v-if="errorsAddUser && errorsAddUser.roles">
                            {{ errorsAddUser.roles[0] }}
                        </p>
                    </div>
                    <template #footer>
                        <Button label="Batal" :disabled="addUserLoading" severity="danger" icon="pi pi-times" outlined @click.prevent="showDialogUser = false" />
                        <Button label="Simpan" :loading="addUserLoading" :disabled="addUserLoading" icon="pi pi-link" @click="handleSubmitAddUser" />
                    </template>
                </Dialog>
                <!-- update -->
                <Dialog v-model:visible="showDialogUpdateUser" @after-hide="clearUpdateUser" :style="{ width: '450px' }" header="Update User" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="name">Nama</label>
                        <InputText id="name" :disabled="updateUserLoading" :invalid="errorsUpdateUser && errorsUpdateUser.name" required="true" autofocus v-model="updateUserData.name" />
                    ...
                    <div class="field">
                        <label for="username">Username</label>
                        <InputText id="username" :disabled="updateUserLoading" :invalid="errorsUpdateUser && errorsUpdateUser.username" required="true" autofocus v-model="updateUserData.username" />
                    ...
                    <div class="field">
                        ...
                        <Dropdown
                            ...
                            placeholder="Pilih Hak Akses"
                            class="w-full"
                            :invalid="errorsUpdateUser && errorsUpdateUser.roles"
                            :disabled="updateUserLoading"
                        />
                        <p class="text-red-500" v-if="errorsUpdateUser && errorsUpdateUser.roles">
                            {{ errorsUpdateUser.roles[0] }}
                        </p>
                    </div>

                    <template #footer>
                        <Button label="Batal" :disabled="updateUserLoading" severity="danger" icon="pi pi-times" outlined @click.prevent="showDialogUpdateUser = false" />
                        <Button label="Update" :loading="updateUserLoading" :disabled="updateUserLoading" icon="pi pi-link" @click="handleUpdateUser" />
                    </template>
                </Dialog>
                <!-- update password -->
                <Dialog v-model:visible="showDialogUpdateUserPassword" @after-hide="clearUpdateUserPassword" :style="{ width: '450px' }" header="Update User Password" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="password">Password Baru</label>
                        <InputText id="password" :disabled="updateUserPasswordLoading" :invalid="errorsUpdateUserPassword && errorsUpdateUserPassword.password" required="true" autofocus v-model="updateUserPasswordData.password" />
                    ...
                    <div class="field">
                        ...
                        <InputText
                            ...
                            :disabled="updateUserPasswordLoading"
                            :invalid="errorsUpdateUserPassword && errorsUpdateUserPassword.confirmation_password"
                            required="true"
                            autofocus
                    ...
                    <template #footer>
                        <Button label="Batal" :disabled="updateUserPasswordLoading" severity="danger" icon="pi pi-times" outlined @click.prevent="showDialogUpdateUserPassword = false" />
