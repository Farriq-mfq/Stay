        ...
        <div class="field">
            ...
            <div class="flex gap-2">
                <Button
                    ...
                    label="Export"
                    @click.prevent="handleExportService"
                    class="mt-3"
                    v-if="sessionId && selectedRombel && filterDate && $can('presences:export-monthly')"
                />
                    ...
                    <tr class="p-row">
                        ...
                        <td class="p-column-body" style="min-width: 100px" v-for="(date, idx) in getRangeDate(presences.data.data.date.startDate, presences.data.data.date.endDate)" :key="date">
                            <div v-if="presence.presences[idx]" class="flex flex-column gap-1">
                                <div class="flex flex-column gap-1" v-if="!(presence.presences[idx][`${format(date, 'dd')}`] && presence.presences[idx][`${format(date, 'dd')}`].leave)">
                                    ...
                                    <Tag severity="danger" v-if="presence.presences[idx][`${format(date, 'dd')}`] && presence.presences[idx][`${format(date, 'dd')}`].allow_twice && presence.presences[idx][`${format(date, 'dd')}`].exit_time">
                                ...
                                <div class="flex flex-column gap-1" v-else>
                                    <Tag severity="warning" v-if="presence.presences[idx][`${format(date, 'dd')}`] && presence.presences[idx][`${format(date, 'dd')}`].leave"> Izin </Tag>
                                </div>
                            </div>
                            <p v-else>-</p>
                        </td>
