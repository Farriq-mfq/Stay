...
<template>
    <div>
        <Dropdown v-if="!multiple" v-model="selectSession" :loading="isLoading" filter @filter="handleFilter" showClear :options="isLoading ? [] : sessions.data.data.items" optionLabel="name" placeholder="Pilih Session" class="w-full"> </Dropdown>
        <MultiSelect
            v-if="multiple"
            display="chip"
